      ...
      <Row gutter="8">
        <Col span="12">
          <Demo title="基础用法" layout="vertical">
            ...
            <div><code>action</code>配置上传接口路径</div>
            <div>{code.base}</div>
          </Demo>
          <Demo title="禁用" layout="vertical">
            ...
            <div>
              <div>设置<code>disabled</code>属性来控制组件是否可用</div>
            </div>
        ...
        <Col span="12">
          <Demo title="附带数据上传" layout="vertical">
            <div>
              <Upload action="/rest/jcmro-sys-service/fileupload/uploadFile" onChange={this.change.bind(this)} onComplite={this.complite.bind(this)} data={data}>
                <Button>上传文件</Button>
              </Upload>
            </div>
            <div>此操作非多此一举，在ajax上传模式中，除了上传文件外，可能会有其他附加参数一起附带表单提交，传参
            <code>data</code>为 Object</div>
            <div>{code.withData}</div>
          </Demo>
          <Demo title="非自动上传模式" layout="vertical">
            <div>
              <Upload action={action} onChange={this.change.bind(this)} onComplite={this.complite.bind(this)} ref="upload" data={data} type="wait">
                <Button>上传文件</Button>
              </Upload>
