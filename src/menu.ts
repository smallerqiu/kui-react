import {
  AlignHorizontalDistributeCenter,
  Anchor,
  AppWindow,
  AppWindowMac,
  ArrowDownNarrowWide,
  ArrowUpToLine,
  BellDot,
  BetweenVerticalEnd,
  Calendar,
  ChartColumnDecreasing,
  ChevronRight,
  CircleDot,
  CirclePlay,
  Columns3,
  Component,
  CreditCard,
  DecimalsArrowRight,
  FileText,
  Flame,
  FolderTree,
  Form,
  GalleryHorizontal,
  Gauge,
  Heart,
  Image,
  Inbox,
  Info,
  Languages,
  LayoutDashboard,
  List,
  ListChevronsUpDown,
  ListCollapse,
  ListFilter,
  Loader,
  Loading,
  Menu,
  MessageCircleCheck,
  MessageCircleMore,
  MessageSquareCheck,
  MessageSquareMore,
  Minus,
  MirrorRectangular,
  MountainSnow,
  PaintBucket,
  Palette,
  PanelLeftClose,
  PanelsTopLeft,
  Pin,
  QrCode,
  RectangleEllipsis,
  SeparatorVertical,
  Settings2,
  ShieldCheck,
  SmilePlus,
  SquareCheck,
  SquareChevronRight,
  SquareDashedText,
  SquareMousePointer,
  Star,
  Table,
  Tag,
  TextAlignStart,
  TextCursorInput,
  ToggleLeft,
  TriangleAlert,
  Upload,
} from "kui-icons";
const navData = [
  {
    title: "menu.start",
    key: "guide",
    children: [
      {
        title: "快速开始",
        sub: "Quick Started",
        name: "quick-started",
        icon: CirclePlay,
      },
      {
        title: "在 Next.js 中使用",
        sub: "Usage With Next.js",
        name: "usage-with-next",
        icon: MountainSnow,
      },
      {
        title: "国际化",
        sub: "Multilingual",
        name: "language",
        icon: Languages,
      },
      { title: "更新日志", sub: "Change Log", name: "change-log", icon: FileText },
      { title: "定制主题", sub: "Themes", name: "theme", icon: Palette },
      { title: "暗色模式", sub: "Dark Mode", name: "dark-mode", icon: PaintBucket },
      {
        title: "组件总览",
        sub: "All Components",
        name: "components",
        icon: Component,
      },
    ],
  },
  {
    title: "menu.universal",
    key: "basic",
    children: [
      { title: "按钮", sub: "Button", name: "button", icon: SquareMousePointer },
      { title: "图标", sub: "Icon", name: "icon", icon: Heart },
    ],
  },
  {
    title: "menu.effects",
    key: "effects",
    children: [
      { title: "涟漪", sub: "Ripple", name: "ripple", icon: CircleDot },
      { title: "火焰边框", sub: "FlameWrap", name: "flame-wrap", icon: Flame },
      // { title: "动效", sub: "Motion", name: "motion", icon: Loader },
    ],
  },
  {
    title: "menu.layout",
    key: "layouts",
    children: [
      { title: "分割线", sub: "Divider", name: "divider", icon: Minus },
      { title: "弹性布局", sub: "Flex", name: "flex", icon: AlignHorizontalDistributeCenter },
      { title: "栅格", sub: "Row Col", name: "row-col", icon: Columns3 },
      { title: "网格", sub: "Grid", name: "grid", icon: LayoutDashboard },
      { title: "布局", sub: "Layout", name: "layout", icon: PanelsTopLeft },
      { title: "页头", sub: "PageHeader", name: "page-header", icon: PanelsTopLeft },
      { title: "列表面板", sub: "ListPanel", name: "list-panel", icon: ListFilter },
      { title: "间距", sub: "Space", name: "space", icon: BetweenVerticalEnd },
      { title: "分隔面板", sub: "Splitter", name: "splitter", icon: SeparatorVertical },
    ],
  },
  {
    title: "menu.navigation",
    key: "navigation",
    children: [
      { title: "锚点", sub: "Anchor", name: "anchor", icon: Anchor },
      { title: "图钉", sub: "Affix", name: "affix", icon: Pin },
      { title: "面包屑", sub: "Breadcrumb", name: "breadcrumb", icon: ChevronRight },
      { title: "下拉菜单", sub: "Dropdown", name: "dropdown", icon: ArrowDownNarrowWide },
      { title: "导航菜单", sub: "Menu", name: "menu", icon: Menu },
      { title: "分页", sub: "Page", name: "page", icon: SquareChevronRight },
      { title: "步骤条", sub: "Steps", name: "steps", icon: ListChevronsUpDown },
      { title: "标签页", sub: "Tabs", name: "tabs", icon: AppWindow },
      { title: "回到顶部", sub: "BackTop", name: "back-top", icon: ArrowUpToLine },
    ],
  },
  {
    title: "menu.form",
    key: "forms",
    children: [
      { title: "输入框", sub: "Input", name: "input", icon: TextCursorInput },
      { title: "自动完成", sub: "AutoComplete", name: "auto-complete", icon: ListChevronsUpDown },
      { title: "标签输入", sub: "InputTag", name: "input-tag", icon: Tag },
      { title: "提及", sub: "Mentions", name: "mentions", icon: MessageSquareMore },
      { title: "一次性密码输入框", sub: "InputOTP", name: "input-otp", icon: ShieldCheck },
      { title: "数字输入框", sub: "InputNumber", name: "input-number", icon: DecimalsArrowRight },
      { title: "多选框", sub: "Checkbox", name: "checkbox", icon: SquareCheck },
      { title: "单选框", sub: "Radio", name: "radio", icon: CircleDot },
      { title: "卡片选择器", sub: "CheckCard", name: "check-card", icon: CreditCard },
      { title: "开关", sub: "Switch", name: "switch", icon: ToggleLeft },
      { title: "选择器", sub: "Select", name: "select", icon: ListChevronsUpDown },
      { title: "树选择器", sub: "TreeSelect", name: "tree-select", icon: FolderTree },
      { title: "日期选择框", sub: "DatePicker", name: "date-picker", icon: Calendar },
      { title: "上传", sub: "Upload", name: "upload", icon: Upload },
      { title: "滑动输入条", sub: "Slider", name: "slider", icon: Settings2 },
      { title: "表单", sub: "Form", name: "form", icon: Form },
    ],
  },
  {
    title: "menu.data",
    key: "data",
    children: [
      { title: "头像", sub: "Avatar", name: "avatar", icon: SmilePlus },
      { title: "卡片", sub: "Card", name: "card", icon: CreditCard },
      { title: "功能卡片", sub: "FeatureCard", name: "feature-card", icon: CreditCard },
      { title: "统计卡片", sub: "StatCard", name: "stat-card", icon: ChartColumnDecreasing },
      { title: "日历", sub: "Calendar", name: "calendar", icon: Calendar },
      { title: "看板", sub: "Kanban", name: "kanban", icon: Columns3 },
      { title: "走马灯", sub: "Carousel", name: "carousel", icon: GalleryHorizontal },
      { title: "折叠面板", sub: "Collapse", name: "collapse", icon: ListCollapse },
      { title: "级联选择", sub: "Cascader", name: "cascader", icon: ListCollapse },
      { title: "描述列表", sub: "Descriptions", name: "descriptions", icon: SquareDashedText },
      { title: "图片", sub: "Image", name: "image", icon: Image },
      { title: "二维码", sub: "QRCode", name: "qrcode", icon: QrCode },
      { title: "表格", sub: "Table", name: "table", icon: Table },
      { title: "穿梭框", sub: "Transfer", name: "transfer", icon: SeparatorVertical },
      { title: "时间轴", sub: "TimeLine", name: "time-line", icon: List },
      { title: "树形控件", sub: "Tree", name: "tree", icon: FolderTree },
      { title: "虚拟列表", sub: "VirtualList", name: "virtual-list", icon: List },
    ],
  },
  {
    title: "menu.notice",
    key: "notices",
    children: [
      { title: "警告提示", sub: "Alert", name: "alert", icon: TriangleAlert },
      { title: "徽标", sub: "Badge", name: "badge", icon: BellDot },
      { title: "抽屉", sub: "Drawer", name: "drawer", icon: PanelLeftClose },
      { title: "空状态", sub: "Empty", name: "empty", icon: Inbox },
      { title: "反馈面板", sub: "FeedbackPanel", name: "feedback-panel", icon: MessageSquareCheck },
      { title: "进度", sub: "Loading", name: "loading", icon: Loading },
      { title: "全局提示", sub: "Message", name: "message", icon: RectangleEllipsis },
      { title: "通知提醒", sub: "Notice", name: "notice", icon: Info },
      { title: "对话框", sub: "Modal", name: "modal", icon: AppWindowMac },
      { title: "气泡卡片", sub: "Poptip", name: "poptip", icon: MessageCircleMore },
      { title: "气泡确认框", sub: "Popconfirm", name: "popconfirm", icon: MessageCircleCheck },
      { title: "进度条", sub: "Progress", name: "progress", icon: Gauge },
      { title: "评分", sub: "Rate", name: "rate", icon: Star },
      { title: "结果", sub: "Result", name: "result", icon: MessageSquareCheck },
      { title: "骨架屏", sub: "Skeleton", name: "skeleton", icon: TextAlignStart },
      { title: "加载中", sub: "Spin", name: "spin", icon: Loader },
      { title: "标签", sub: "Tag", name: "tag", icon: Tag },
      { title: "文字提示", sub: "Tooltip", name: "tooltip", icon: MessageSquareMore },
      { title: "漫游式引导", sub: "Tour", name: "tour", icon: SquareMousePointer },
      { title: "水印", sub: "Watermark", name: "watermark", icon: MirrorRectangular },
    ],
  },
  {
    title: "menu.other",
    key: "other",
    children: [
      {
        title: "颜色",
        sub: "ColorPicker",
        name: "color-picker",
        icon: Palette,
      },
    ],
  },
];

const routeData = navData.flatMap(({ key, children }) =>
  children.map((item) => ({ ...item, key })),
);

export type RouteItem = (typeof routeData)[number];
export { navData, routeData };
