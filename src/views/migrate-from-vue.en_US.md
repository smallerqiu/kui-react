# Migrating from kui-vue

This guide covers component-library migration from **kui-vue** to **react-kui**, using the APIs in **react-kui 3.x / kui-vue 6.x**. It does not teach how to rewrite an application between React and Vue. Similar component names do not imply identical bindings, callbacks, or render functions.

## 1. Installation and styles

Install `react-kui` in the target application. React requires React 19.2+; Vue requires Vue 3.5+. Check the version you actually install: package version numbers and release schedules are independent.
...
Change locale imports from `kui-vue/locale/zh-CN` to `react-kui/locale/zh-CN`, and load the relevant dayjs locale. Review CSS variables and DOM-dependent overrides. Scoped styles and CSS Modules do not automatically reach all portaled overlays; check popup containers and local themes.
