...
- Row 默认使用 Flex 布局，其子元素可通过 `start`、`center`、`end`、`space-between`、`space-around` 排列。
...
| 属性 | 说明 | 类型 | 默认值 |
| --- | --- | --- | --- |
| align | flex 布局下的垂直对齐方式：`top` `middle` `bottom` | `"top" \| "bottom" \| "middle"` | `top` |
| justify | flex 布局下的水平排列方式：`start` `end` `center` `space-around` `space-between` | `"center" \| "space-around" \| "space-between" \| "end" \| "start"` | `start` |
| gutter | 栅格间距，单位 px，左右平分；使用数组形式同时设置 `[水平间距, 垂直间距]` | `number \| [number, number]` | - |
...
| flex | Flex 布局填充，例如 `1`、`auto`、`100px`、`1 1 200px` | `string \| number` | - |
| xs | `<576px`，数字表示 span，也支持响应式对象 | `ColResponsiveSize` | - |
