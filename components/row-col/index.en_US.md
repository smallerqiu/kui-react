...
| justify | Horizontal arrangement in flex layout: `start` `end` `center` `space-around` `space-between` | `"center" \| "space-around" \| "space-between" \| "end" \| "start"` | `start` |
| gutter | Grid spacing, in px, evenly distributed left and right. Use `[horizontal, vertical]` for two-axis spacing | `number \| [number, number]` | - |
