...
| size | 值为`small`，展示小尺寸 | `"small" \| "medium" \| "large"` | - |
| format | 自定义进度条文字 | `((percent: number) => React.ReactNode)` | - |
| status | 进度条状态,提供四种类型: `active`,`exception`,`success`,`normal` | `"normal" \| "active" \| "success" \| "exception"` | normal |
| type | 进度条类型,提供三种类型: `line`,`circle`,`dashboard` | `"circle" \| "line" \| "dashboard"` | - |
