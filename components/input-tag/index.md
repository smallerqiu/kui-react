...
[受控标签](./demo/controlled.tsx)

- 使用 `value` 和 `onChange` 管理标签集合。

[数量限制](./demo/limit.tsx)

- 使用 `max` 限制最多可输入的标签数，使用 `maxTagCount` 限制展示数量。
...
| clearable | 是否显示一键清空按钮 | `boolean` | true |
...
| onRemove | 删除标签 | `((value: string, index: number) => void)` | - |
| onClear | 清空标签 | `(() => void)` | - |
