...
| shape | Avatar shape | `"round" \| "square" \| "circle"` | circle |
| size | Avatar size | `AvatarSize` | default |
| src | Image source | `string` | - |
| alt | Alternative text when the image cannot be displayed | `string` | - |
| onError | Image error callback; return `false` to prevent rendering fallback | `((event: React.SyntheticEvent<HTMLImageElement>) => boolean \| void)` | - |
...
| shape | Sets the shape of all child avatars | `"round" \| "square" \| "circle"` | circle |
