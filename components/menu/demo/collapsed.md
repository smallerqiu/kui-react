...
```tsx
import {Button, Menu ,SubMenu } from 'react-kui';

const Demo = ()=> {
  const [collapsed,toggle] = React.useState(false)
  return(
    <div style={{width:256}}>
      <Button onClick={()=>toggle(!collapsed)} icon={collapsed?'list':'menu'} type="primary"></Button>
      <br/>
      <br/>
      <Menu selectedKeys={['1-1']} openKeys={['sub2']} theme="dark" inlineCollapsed={collapsed} mode="inline">
