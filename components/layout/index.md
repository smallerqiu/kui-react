...
[现代企业级管理后台 (Enterprise Admin Console)](./demo/enterprise-admin-console.tsx)
...
[响应式门户/官网布局 (Portal Exhibition)](./demo/portal-exhibition.tsx)

- 适用于信息量较大但层级较浅的门户网站、展示型官网或个人博客。
- 布局逻辑：单一垂直方向的 Layout，直接排布 Header、Content、Footer。
- 搭配组件：Menu（水平模式）、Carousel（轮播图）、Card（内容卡片）。

[文档/知识库布局 (Documentation Layout)](./demo/documentation-layout.tsx)
...
[任务工作台 (The "Workbench" Details Layout)](./demo/workbench-layout.tsx)

- 左侧导航 + 中间看板 + 右侧详情面板 (Right Sider)。

[协作聊天布局 (Messaging & Collaboration)](./demo/messaging-collaboration.tsx)

- 场景： 客服系统、Slack 镜像、应用内私信。
- 核心点： 内容区（Content）负责消息流，页脚（Footer）作为固定高度的输入区域。
- 这个案例展示了如何让 Footer 承载复杂的交互（文本框、表情按钮、发送键）。

[可视化编辑器布局 (IDE / Low-code Layout)](./demo/low-code-layout.tsx)
