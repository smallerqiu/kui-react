...
[仪表盘卡片布局 (Auto-fill + Min-Width)](./demo/auto-fill-min-width.tsx)
...
[复杂表单响应式 (Breakpoint Fallback)](./demo/breakpoint-fallback.tsx)

- 断点的向上/向下寻找逻辑。例如：定义了 md 没定义 lg，系统是否能正确应用 md 的值。

[圣杯布局/管理后台 (Fixed Rows & Areas)](./demo/fixed-rows-areas.tsx)

- `rows` 和 `rowSpan` 的垂直控制力。

[响应式隐藏与强制排序 (Suffix & Display None)](./demo/suffix-display-none.tsx)

- `span: 0` 彻底移除 DOM 占位，`suffix` 跨越所有动态项。

[图片画廊 / 瀑布流（Bento Grid 风格）](./demo/bento.tsx)

- 非对称布局

[复杂数据详情页（对齐与留白）](./demo/align-white-space.tsx)

- 描述列表（Descriptions）的效果，但在不同屏幕下自动调整展示密度。

[Hero Section 叠加布局（Layering）](./demo/hero-section.tsx)

- `grid-column-start` 和 `grid-row-start` 的重叠能力,文字浮动在图片某处

[响应式底栏 / 菜单（Footer Strategy）](./demo/footer-strategy.tsx)

- 从“多列垂直排列”到“单列折叠”的极致压缩。

[布局层级设计 (Architecture)](./demo/architecture.tsx)
...
| debug        | 调试模式。开启后会显示红色透明背景列，方便开发者校对布局。        | bool                      | false  |
...
| suffix  | 尾部固定。设为 true 时，该项将强制移动到当前行的末尾。     | bool                 | false  |
