  ...
  useEffect(() => {
    ...
    const createPattern = () =>
      new Promise<string>((resolve) => {
        ...
        source.onload = () => render(source);
        source.onerror = () => render();
        source.src = image;
      });
    ...
    const install = async () => {
      ...
      const cellWidth = width + gap[0],
        cellHeight = height + gap[1],
        staggered = layout === "stagger";
      Object.assign(inner.style, {
        width: "100%",
        height: "100%",
        backgroundImage: `url(${pattern})`,
        backgroundRepeat: "repeat",
        backgroundSize: `${cellWidth * (staggered ? 2 : 1)}px ${cellHeight * (staggered ? 2 : 1)}px`,
        pointerEvents: "none",
      });
      root.attachShadow({ mode: "closed" }).appendChild(inner);
      target.appendChild(root);
      watermarkRef.current = root;
      restoringRef.current = false;
      if (!antiTamper) return;
      const restore = () => {
        if (!disposed && !restoringRef.current) {
